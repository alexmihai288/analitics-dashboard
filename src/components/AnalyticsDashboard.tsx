...
const Badge = ({ percentage }: { percentage: number }) => {
  ...
  return (
    <span
      className={`inline-flex gap-1 items-center rounded-md px-2 py-1 text-xs font-medium right-1 ring-inset ${
        isPositive
          ? positiveClassname
          : isNeutral
          ? neutralClassname
...
export const AnalyticsDashboard: FC<AnalyticsDashboardProps> = ({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card className="flex flex-col sm:grid-rows-4 gap-6">
        ...
        <div className="col-span-3 flex items-center justify-between flex-wrap gap-8">
          {topCountries?.map(([countryCode, number], i) => {
            return (
              <div
                key={i}
